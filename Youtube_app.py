...
# Function to create a Database Connection using Credentials from Streamlit Secrets
...
# Function to check if Channel exists in MySQL Database
def channel_exists(cur, channel_id):
    query = 'Select channel_id from channels where channel_id = %s'
    cur.execute(query, (channel_id,))    # Denote channel_id is a single-element tuple hence , is critical
    return cur.fetchone() is not None


# Function to Harvest Youtube Data and Store in MYSQL Database
def extract_insert_data_st(channel_id):
    if st.button("Extract Data and Store",type='primary'):
...
def main():
    ...
    # Check if the Channel exists in MySQL Database
    if channel_id:
        if channel_exists(cur, channel_id):
            st.success("Channel exists in the Data Warehouse already .....!")
        else:
            extract_insert_data_st(channel_id)
    ...
    # Display Channel Statistics in the Streamlit App
    count = st.markdown(
            """
